...
function FormAnswerRequest() {
  ...
  return (
   ...
   <div className='rounded-md mt-5 m-5 h-full w-[600px] min-w-max mx-auto p-5 border border-t-blue-500 bg-white'>
       <div className='flex justify-between items-center border-teal-500 '>
       <input type="email" placeholder={question.Email} id={question.Email} onChange={(e)=>{setAnswerData((prev)=>({...prev, [e.target.id]:e.target.value}))}}  className=' appearance-none bg-transparent border-none w-full text-gray-700 mr-3 py-1 px-2 leading-tight focus:outline-none focus:border-b border-teal-500' required="required"/>  
     </div>
 ...
 <div className='flex justify-between items-center gap-4'> 
  <label className='text-xl font-medium flex items-center justify-between gap-4'>{item.questionName} </label>  
    ...
    <div className='flex justify-between items-center m-5'>
    <item.questionType id={item.questionName} required={`${item.required}`} onChange={(e)=>{setAnswerData((prev)=>({...prev, [e.target.id]:e.target.value}))}} className='bg-transparent w-full text-gray-700 mr-3 py-1 px-2 leading-tight focus:outline-none border-0 hover:border-b-2 focus-visible:border-b border-teal-500'>
    {item.questionOption.map((option,inde)=>(
    ...
    item.questionType=="rating" ?
      <div className='flex justify-between items-center m-5 w-2'>
        ...
        return(
          <label>
             <input type="radio" value={currentRating} id={item.questionName} required={`${item.required}`} onChange={(e)=>{handleRating(e.target.id,currentRating)}} className='appearance-none bg-transparent text-gray-700 leading-tight focus:outline-none border-0 focus-visible:border-b border-teal-500'/>  
